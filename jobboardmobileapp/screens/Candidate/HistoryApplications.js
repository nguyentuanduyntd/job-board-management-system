...
const AppCard = React.memo(({ item, navigation, openPriorityModal, setCancelTarget, getHighlightAnim }) => {
...
const EmptyState = React.memo(({ activeTab, navigation }) => (
...
const PriorityModal = React.memo(({ visible, priorityTarget, loadingPkgs, packages, selectedPkg, setSelectedPkg, purchasing, handlePurchase, closePriorityModal }) => {
    const jobTitle = typeof priorityTarget?.job === 'object' ? priorityTarget?.job?.title : '—';

    return (
        <Modal transparent visible={visible} animationType="slide" onRequestClose={closePriorityModal}>
...
const CancelModal = React.memo(({ cancelTarget, cancelling, doCancel, onClose }) => {
    if (!cancelTarget) return null;
    const jobTitle = typeof cancelTarget?.job === 'object' ? cancelTarget?.job?.title : '—';

    return (
        <Modal transparent visible={!!cancelTarget} animationType="fade" onRequestClose={onClose}>
            <View style={styles.overlay}>
                <View style={styles.dialog}>
                    ...
                    <View style={styles.dialogBtns}>
                        <TouchableOpacity style={styles.dialogBtnSecondary} onPress={onClose} disabled={cancelling}>
...
export default function MyApplications({ navigation }) {
    ...
    const getHighlightAnim = useCallback((id) => {
    ...
    const openPriorityModal = useCallback(async (application) => {
    ...
    const handlePurchase = async () => {
        ...
        try {
            ...
            const targetLevel = selectedPkg.level;
            ...
            Alert.alert(
                ...
                `Hồ sơ của bạn đang được hệ thống nâng cấp lên ưu tiên ${LEVEL_LABELS[targetLevel]}.`
            );
        } catch (ex) {
    ...
    const doCancel = async () => {
        if (!cancelTarget || !user?.token) return;
        setCancelling(true);
        try {
            const api = authApi(user.token);
    ...
    const renderAppItem = useCallback(({ item }) => (
        <AppCard 
            ...
            getHighlightAnim={getHighlightAnim}
        />
    ), [navigation, openPriorityModal, getHighlightAnim]);
        ...
        <SafeAreaView style={styles.container}>
            ...
            {loading ? (
                ...
                <FlatList
                    ...
                    ListEmptyComponent={<EmptyState activeTab={activeTab} navigation={navigation} />}
            ...
            <PriorityModal 
                ...
                closePriorityModal={closePriorityModal}
            />
